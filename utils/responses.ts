...
export function invalidAlias(): Response {
  return error("Invalid URL alias", Status.BadRequest);
}

export function aliasNotFound(): Response {
  return error("URL lias not found", Status.BadRequest);
}

export function couldNotAuthenticate(): Response {
  return error("Could not authenticate with SSO Dashboard", Status.Unauthorized);
}
